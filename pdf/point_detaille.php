...
$sql_inter = mysql_query("SELECT * FROM intervention, client, technicien 
    WHERE intervention.idclient = client.idclient 
    AND intervention.employe = technicien.idtechnicien
...
<page id="page">
    ...
    <div id="corps">
        ...
        <table style="float: right">
            <tr>
                <td>Point détaillé:</td>
                <td>Edité le <?php echo $date; ?> à <?php echo $heure; ?></td>
            </tr>
        </table>
        ...
        <div class="box_symptome">
            <div class="box_symptome_header">Symptome & Remarque</div>
            <div class="box_symptome_corps">
                <?php echo $donnee_inter['symptome']; ?><br><br>
                <?php echo $donnee_inter['remarques']; ?>
            </div>
        </div>
        <div class="box_devis">
            <div class="box_devis_header">Opération réalisée par:
                <?php echo $donnee_inter['prenom_technicien']; ?>
            </div>
            <div class="box_devis_corps">
                <?php
                    $sql_tache = mysql_query("SELECT * FROM tache_intervention WHERE idintervention = $idintervention");
                    while($donnee_tache = mysql_fetch_array($sql_tache) )
                    {  
                ?>
                <?php echo $donnee_tache['description_tache']; ?><br>
                <?php } ?>
            </div>
        </div>
        <br>
        ...
        <br>
        <div class="box_info_complementaire">
            <div class="box_info_complementaire_header">Suivi</div>
            <div class="box_info_complementaire_corps">
            <?php
                    $sql_suivi = mysql_query("SELECT * FROM suivi_intervention WHERE idintervention = $idintervention");
                    while($donnee_suivi = mysql_fetch_array($sql_suivi))
                    {   
                ?>
                <div style="color: blue"><?php echo $donnee_suivi['libelle']; ?></div>
                <?php } ?>
            </div>
        </div>
